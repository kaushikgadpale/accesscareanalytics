...
async def fetch_forms_responses(form_id):
    ...
    try:
        ...
        # Get form details
        form = await graph_client.users.by_user_id("me").onenote.form_id(form_id).get()
        if not form:
            st.warning(f"Form with ID {form_id} not found")
            return None
        
        # Get form responses
        responses = await graph_client.users.by_user_id("me").onenote.form_id(form_id).responses.get()
...
def render_forms_tab():
    ...
    fetch_form_button = st.button("🔄 Fetch Form Responses")
